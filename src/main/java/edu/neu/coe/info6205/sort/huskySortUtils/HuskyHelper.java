...
public class HuskyHelper<X extends Comparable<X>> implements Helper<X> {
     ...
     */
    public HuskyHelper(final Helper<X> helper, final HuskyCoder<X> coder, final Consumer<X[]> postSorter, final boolean makeCopy, final Config config) {
        this.helper = helper;
        this.coder = coder;
     ...
     * @param makeCopy    explicit setting of the makeCopy value used in sort(Y[] xs)
     */
    public HuskyHelper(final String description, final int n, final HuskyCoder<X> coder, final Consumer<X[]> postSorter, final long seed, final boolean makeCopy, final Config config) {
        this(new BaseHelper<>(description, n, seed, config), coder, postSorter, makeCopy, config);
    }
     ...
     * @param n           the number of elements expected to be sorted. The field n is mutable so can be set after the constructor.
     */
    public HuskyHelper(final String description, final int n, final HuskyCoder<X> coder, final Consumer<X[]> postSorter, final Config config) {
        this(description, n, coder, postSorter, System.currentTimeMillis(), false, config);
    }
